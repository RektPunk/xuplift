...
use xuplift::metalearners::rlearner::{RClassifier, RRegressor};
...
fn test_rclassifier() {
    ...
    let rclassifier = RClassifier::new(
        x.as_ref(),
        t.as_ref(),
        y.as_ref(),
        &is_categorical,
    ...
    let uplift_estimate = rclassifier.predict_uplift(x.as_ref());
    ...
    let uplift_explanation = rclassifier.explain_uplift(x.as_ref());
    assert_eq!(uplift_explanation.ncols(), n_features);

    let base_value = rclassifier.tau.base_value;

...
/// Cross Classifier for Uplift Modeling.
///
/// # Reference
...
impl XClassifier {
    ...
    /// * `mu_max_iter` - Maximum iterations for the outcome model solver.
...
/// Cross Regressor for Uplift Modeling.
pub struct XRegressor {
    /// Imputed effect models
    pub tau_t1: Regressor,
    pub tau_t0: Regressor,
...
impl XRegressor {
    /// Initializes and fits the XRegressor using the provided data.
    ///
    /// # Arguments
    /// * `x` - Feature matrix (n_samples x n_features).
    /// * `t` - Treatment vector (n_samples).

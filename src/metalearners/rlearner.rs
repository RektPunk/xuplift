...
/// Residual Classifier for Uplift Modeling.
///
/// # Reference
...
impl RClassifier {
    /// Initializes and fits the RClassifier using the provided data.
    ///
    /// # Arguments
...
/// Residual Regressor for Uplift Modeling.
pub struct RRegressor {
    /// Treatment effect model
    pub tau: Regressor,
}

impl RRegressor {
    /// Initializes and fits the RRegressor using the provided data.
    ///
    /// # Arguments
    /// * `x` - Feature matrix (n_samples x n_features).
    /// * `t` - Treatment vector (n_samples).
